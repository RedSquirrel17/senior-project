...
void LCD_PIN_Init(void){
	...
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOAEN; // Enable the clock to GPIO Ports A, B, C, D
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOBEN;
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOCEN;
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIODEN;
	
	GPIOA->MODER &= ~(0xC03FF000) ; // Configure Port A
	GPIOA->MODER |= (0x802AA000) ;
	
	GPIOA->AFR[0] &= ~(0xFF000000) ; // Set Alternate Function Port A
	GPIOA->AFR[0] |= (0xBB000000) ;
	GPIOA->AFR[1] &= ~(0xF0000FFF) ;
	GPIOA->AFR[1] |= (0xB0000BBB) ;
	
	GPIOB->MODER &= ~(0xFF0C0F0F) ; // Configure Port B
	GPIOB->MODER |= (0xAA080A0A) ;
	
	GPIOB->AFR[0] &= ~(0x00FF00FF) ; // Set Alternate Function Port B
	GPIOB->AFR[0] |= (0x00BB00BB) ;
	GPIOB->AFR[1] &= ~(0xFFFF00F0) ;
	GPIOB->AFR[1] |= (0xBBBB00B0) ;
	
	GPIOC->MODER &= ~(0x0003FFC0) ; // Configure Port C
	GPIOC->MODER |= (0x0002AA80) ;
	
	GPIOC->AFR[0] &= ~(0xFFFFF000) ; // Set Alternate Function Port C
	GPIOC->AFR[0] |= (0xBBBBB000) ;
	GPIOC->AFR[1] &= ~(0x0000000F) ;
	GPIOC->AFR[1] |= (0x0000000B) ;
	
	GPIOD->MODER &= ~(0xFFFF0000) ; // Configure Port D
	GPIOD->MODER |= (0xAAAA0000) ;
	
	GPIOD->AFR[0] &= ~(0x00000000) ; // Set Alternate Function Port D
...
void LCD_DisplayString(uint8_t* ptr){
	char i = 0 ;
	for(i = 0; ptr[i] !='\0'; ++i) { // Uses counter to keep place holder for the position of the string
		if(i <= 5) {
			LCD_WriteChar(&ptr[i] , 0, 0, i); // Calls LCD_WriteChar function
		}
	}
}

void LCD_Display_Name(void){
	while(LCD->SR & LCD_SR_UDR) {;} // Sets the UDR flag of LCD_SR register to request update display
	LCD->RAM[0] |= 0x07C0F178 ; // Sets segments of six different characters, in particular "B A R T A S"
	LCD->RAM[1] |= 0x0000000C ;
	LCD->RAM[2] |= 0x17C27338 ;
	LCD->RAM[3] |= 0x00000005 ;
	LCD->RAM[4] |= 0x90000000 ;
	LCD->RAM[5] |= 0x00000000 ;
	LCD->RAM[6] |= 0x00008000 ;
	LCD->RAM[7] |= 0x00000001 ;
	LCD->SR |= LCD_SR_UDR ; // Updates UDR flag to state that display has been updated
}

void LCD_Configure(void){
	LCD->CR &= ~(LCD_CR_BIAS) ; // Clears bias register
	LCD->CR |= LCD_CR_BIAS_1 ; // Sets bias to 1/3
	LCD->CR &= ~(LCD_CR_DUTY) ; // Clears duty register
	LCD->CR |= LCD_CR_DUTY_1 | LCD_CR_DUTY_0 ; // Sets duty to 1/4
	LCD->FCR |= LCD_FCR_CC ; // Sets contrast to max value
	LCD->FCR |= LCD_FCR_PON ; // Sets pulse period
	LCD->CR &= ~(LCD_CR_MUX_SEG); // Enables the mux segment of the LCD_CR
	LCD->CR &= ~(LCD_CR_VSEL); // Selects internal voltage as LCD voltage source
	while((LCD->SR & LCD_SR_FCRSR) == 0) {;} //Waits until FCRSF flag of LCD_CR is set
	LCD->CR |= LCD_CR_LCDEN ; // Enables LCD
	while((LCD->SR & LCD_SR_ENS) == 0) {;} // Waits until LCD is enabled
	while((LCD->SR & LCD_SR_RDY) == 0) {;} // Waits until LCD booster is ready
}
...
/*
// Function to print a string and scroll it across the LCD	
void LCD_Message(uint8_t* string) {
	long timer = 0;
	uint8_t done[] = " DONE";
	uint8_t Size = String_Size(string);
	uint8_t segment[] = "MOTOR IS TURNING CLOCKWISE";
	uint8_t i = 0;
	String_Position = 0;
	
	for(i = 0; i <= (Size + 5); i++) {
		String_Shift(string, segment);
		LCD_DisplayString(segment);
		timer = 5000000;
		while(timer != 0) timer--;
		LCD_Clear();
	}
...
void String_Shift(uint8_t* string, uint8_t* segment) {
	char i = 0;
	// Set first 5 segments to the segment in the immediate right position
	for(i = 0; i < 6; i++) {
		segment[i] = segment[i+1];
	}
	segment[6] = (string[String_Position] == '\0'?' ' :string[String_Position]);
	String_Position++;
}

uint8_t String_Size(uint8_t* string) {
	uint8_t size = 0;
	while(string[size] != '\0') {
		size++;
	}
